   ...
   public class Producer
   {
      public async Task Produce(CancellationToken cancellationToken)
      {
         ...
         using (var schemaRegistry = new CachedSchemaRegistryClient(schemaRegistryConfig))
            using (var producer =
                new ProducerBuilder<int, TestEvent>(producerConfig)
                    .SetKeySerializer(new AvroSerializer<int>(schemaRegistry))
                    .SetValueSerializer(new AvroSerializer<TestEvent>(schemaRegistry))
                    .Build())
            {
                int i = 0;
                ...
                {
                   i++;
                   TestEvent user = new TestEvent() { Name = "Messsage: " + i, FavoriteColor = "read", FavoriteNumber = i, HourlyRate = new Avro.AvroDecimal(67.99)};
                   ...
                   //var result = await new AvroSerializer<TestEvent>(schemaRegistry).SerializeAsync(user, new SerializationContext());
                    ...
                    await producer
                        .ProduceAsync("Test", new Message<int, TestEvent> { Key = i, Value = user }, cancellationToken)
                        .ContinueWith(task =>
                            {
                                if (!task.IsFaulted)
                                {
